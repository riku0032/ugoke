...
# --- 4. 通常モード ---
    ...
    img_file = st.camera_input("写真を撮って解析")
    if img_file:
...
# --- 5. ゲームモード ---
    ...
    if st.session_state.game_status == "IDLE":
        ...
        st.markdown(f'<div class="game-card"><h2>お題: <span style="color:#00f2fe">{st.session_state.target_pose}</span></h2></div>', unsafe_allow_html=True)
        if st.button("撮影スタート！"):
        ...
        # カウントダウンHTMLを注入
        st.markdown('<div id="countdown-container"><div id="countdown-display"></div></div>', unsafe_allow_html=True)
        # JS起動
        components.html(f"<script>{js_code} startCountdown();</script>", height=0)
